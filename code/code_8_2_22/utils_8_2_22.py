...
def predict_from_mat2(B, X_test, Y_test):
    values = Y_test.unique()
    k = values.shape[0]
    T = B[0:k-1,:]
    W = B[k-1:,:]
...
def predict_from_mat3(B, X_test, Y_test):
    T = B[0:2,:]
    W = B[2:,:]
    TASK_num = Y_test.shape[0]
    for i in range(TASK_num):
        X_hidden = X_test[i, :, :] #1x1289x275

        # unique_theta = torch.sort(torch.unique(self.T[:, i]))
        unique_theta = torch.cat((torch.sort(torch.unique(T[:, i])).values, torch.tensor(torch.inf).unsqueeze(0)),
                                 dim=-1)
        # unique_theta.values[-1]=torch.inf
        out = torch.matmul(X_hidden, W[:, i])
        tmp = out[:, None].repeat(1, unique_theta.shape[0])
        pred = torch.argmax((tmp < unique_theta).long(), dim=1)+1
        if i == 0:
            Y_pred = pred.unsqueeze(0)
        else:
            Y_pred = torch.cat((Y_pred, pred.unsqueeze(0)), 0)
